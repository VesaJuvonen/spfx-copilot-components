...
export async function createDeploymentMarker({ distRoot, commitSha }) {
  ...
  assert(Array.isArray(catalog.components), 'Built catalog does not contain a components array');
  ...
  await writeFile(
    path.join(distRoot, 'deployment.json'),
    `${JSON.stringify(marker, null, 2)}\n`,
  );
  return marker;
}

async function verifyAttempt({ siteUrl, expectedCommitSha, attempt, fetchImpl }) {
  ...
  assert(catalog.components.length === marker.componentCount, 'Deployed catalog count does not match the build marker');
  assert(catalog.generatedAt === marker.generatedAt, 'Deployed catalog timestamp does not match the build marker');

  const targets = [new URL('.', baseUrl)];
  for (const component of catalog.components) {
    targets.push(new URL(`samples/${encodeURIComponent(component.slug)}/`, baseUrl));
    targets.push(new URL(component.preview.src640, baseUrl));
  }
  await Promise.all(targets.map((target) => checkedFetch(
    fetchImpl,
    target,
    expectedCommitSha,
    attempt,
...
export async function verifyDeployment({
  ...
  assert(siteUrl, 'SITE_URL is required');
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      const marker = await verifyAttempt({ siteUrl, expectedCommitSha, attempt, fetchImpl });
      console.log(`Verified ${marker.componentCount} components from deployment ${marker.commitSha}`);
      return marker;
    } catch (error) {
      lastError = error;
...
async function run() {
  ...
  if (command === 'create') {
    ...
    console.log(`Recorded ${marker.componentCount} components for deployment ${marker.commitSha}`);
    return;
  }
  if (command === 'verify') {
