...
const agentZipPath = path.join(root, 'sharepoint', 'solution', 'debug', 'ClientSideAssets', 'zava-employee-agent.zip');
...
function assertParameter(parameter, label, simple = false) {
  ...
  if (parameter.enum !== undefined) {
    assert(parameter.type === 'string' && Array.isArray(parameter.enum) && parameter.enum.length > 0, `${label}.enum is invalid.`);
  }
...
for (const [key, limit] of Object.entries(metadataLimits)) {
  if (plugin[key] !== undefined) assertString(plugin[key], key, limit);
}
